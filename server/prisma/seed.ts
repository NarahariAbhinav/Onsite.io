import { PrismaClient, Role, SopLifecycleStatus, Severity, ProjectStatus, AuditStatus, IssueStatus, CapaStatus } from "@prisma/client";
...
async function main() {
  ...
  const defaultPassword = await bcrypt.hash("siteflow2026!", 10);

  const usersData = [
    { email: "r.menon@siteflow.internal", fullName: "R. Menon", role: Role.SYSTEM_ADMIN, dept: "Management", desig: "Project Admin" },
    { email: "a.sharma@siteflow.internal", fullName: "A. Sharma", role: Role.SOP_OWNER, dept: "Civil Engineering", desig: "Chief Process Engineer" },
    { email: "k.iyer@siteflow.internal", fullName: "K. Iyer", role: Role.QUALITY_MANAGER, dept: "Quality & Assurance", desig: "QA/QC Lead" },
    { email: "s.deshmukh@siteflow.internal", fullName: "S. Deshmukh", role: Role.PROJECT_MANAGER, dept: "Projects", desig: "Senior Project Manager" },
    { email: "v.patil@siteflow.internal", fullName: "V. Patil", role: Role.EMPLOYEE_LEARNER, dept: "Civil Execution", desig: "Junior Site Engineer" },
    { email: "m.joshi@siteflow.internal", fullName: "M. Joshi", role: Role.AUDITOR, dept: "Compliance", desig: "Lead Auditor" },
  ];
  ...
  const sopsData = [
    {
      ...
      audit: {
        ...
        checkpoints: [
          { checkpointText: "Perimeter barricading and warning tape installed", expectedControl: "Continuous 1.2m rigid guardrail around pit", severity: Severity.CRITICAL },
          { checkpointText: "Trench shoring integrity verified", expectedControl: "Strut deflection within allowable 5mm limits", severity: Severity.HIGH },
        ],
      },
    },
    {
      ...
      audit: {
        ...
        checkpoints: [
          { checkpointText: "Slump test conducted for every 30m³ batch", expectedControl: "Logged slump between 100-140mm", severity: Severity.HIGH },
          { checkpointText: "Formwork plumb and leak-tight", expectedControl: "Zero slurry leakage at base seams", severity: Severity.CRITICAL },
        ],
      },
    },
  ];
    ...
    if (!existingSop) {
      const sop = await prisma.sop.create({
        data: {
          ...
          ownerId: createdUsers[Role.SOP_OWNER].id,
      ...
      const version = await prisma.sopVersion.create({
        data: {
          ...
          createdById: createdUsers[Role.SOP_OWNER].id,
  ...
  for (const p of projectsData) {
    await prisma.project.upsert({
      ...
      create: {
        organizationId: org.id,
        managerId: createdUsers[Role.PROJECT_MANAGER].id,
        ...p,
      },
      update: {},
    });
